...
const CaseStudyList: React.FC<CaseStudyListProps> = ({ props }) => {
  ...
  const categories = ["All", ...new Set(props.data.map((caseStudy) => caseStudy.category))];

  const filteredData =
    selectedCategory === "All"
      ? props.data
      : props.data.filter((caseStudy) => caseStudy.category === selectedCategory);
  ...
  return (
    <div className={styles.caseslide} id="case-slide">
      ...
      <button className={`${styles.tabButton} ${styles.viewbtn} ${showAll ? styles.activeTab : ""} mx-auto`} onClick={() => setShowAll(!showAll)}>
